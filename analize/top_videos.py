...
top = df.groupBy(["title", "publish_time"]).count().sort("count", ascending=False).limit(10)
...
cd = df.where(cond_udf(df.title)).select(["title", "publish_time", "trending_date", "views", "likes", "dislikes", "comment_count"])
